...
// WSAAsyncSelect 모델을 사용하면 소켓과 관련된 네트워크 이벤트를 윈도우 메시지 형태로 받는다.
// 모든 소켓과 관련된 메시지가 하나의 윈도우 프로시저(하나의 스레드)에 전달된다.
// 동기 입출력 방식과 비동기 통지 방법을 사용하여 다중 처리를 제공한다.

// Select 함수처럼 WSAAsyncSelect함수도 소켓과 관련된 이벤트를 알려줄 뿐 소켓 정보를 관리해주지 않는다. 
